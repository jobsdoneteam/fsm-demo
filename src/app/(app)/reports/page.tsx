...
export default async function ReportsPage() {
  ...
  const [jobs, invoices, timeEntries, employees, serviceTypes] = await Promise.all([
    prisma.job.findMany({ where: { tenantId }, include: { assignedTo: true } }),
    ...
    prisma.serviceType.findMany({ where: { tenantId }, include: { _count: { select: { jobs: true } } } }),
  ])
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          ...
          <table className="data-table text-sm">
            <thead><tr><th>Technician</th><th>Jobs</th><th>Completed</th><th>Hours</th><th>Rate</th></tr></thead>
            <tbody>
              {byTech.filter(t => t.jobs > 0).map(t => (
                <tr key={t.name}>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          ...
          <div className="space-y-3">
            {serviceTypes.sort((a,b) => b._count.jobs - a._count.jobs).map(st => (
